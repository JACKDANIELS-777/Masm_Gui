;CreateLabelEditThread is added
; Still a work in progress settting up multithreading correctly to allow for maximum efficiency.
...
CreateLabelEditThread proc

_loop:

    pause
    cmp byte ptr[ThreadPtr],0
    je _loop
    ...
    mov rcx, qword ptr[ThreadData]

    mov rcx,0

    mov rdx, qword ptr[ThreadData+8]
    mov r8, qword ptr[ThreadData+16]
    mov r9, qword ptr[ThreadData+24]

    mov rax, qword ptr[ThreadData+32]
    mov qword ptr[rsp+20h],rax  ;r10

    mov rax, qword ptr[ThreadData+40]
    mov qword ptr[rsp+28h],rax ;r11

    mov rax, qword ptr[ThreadData+48]
    mov qword ptr[rsp+30h],rax ;r12

    mov rax, qword ptr[ThreadData+56]
    mov qword ptr[rsp+38h],rax ;r13

    mov rax, qword ptr[ThreadData+64]
    mov qword ptr[rsp+40h],rax ;r14

    mov rax, qword ptr[ThreadData+72]
