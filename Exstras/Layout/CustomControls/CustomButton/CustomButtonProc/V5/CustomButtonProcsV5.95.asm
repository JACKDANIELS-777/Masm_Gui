; =========================================================================================
; 
; =========================================================================================

include CustomButtonConstants.inc

extern FrameRect:proc
extern CreateSolidBrush:proc
extern Color32:proc
extern DeleteObject:proc
extern FillRect:proc
extern GradientFill:proc
extern GetLastError:proc
extern AlphaBlend:proc
extern CreateCompatibleDC:proc
extern DeleteObject:proc
extern DeleteDC:proc
extern CreateCompatibleBitmap:proc
extern SelectObject:proc
extern FillRect:proc
extern SetTextColor:proc
extern SetBkMode:proc
extern GetWindowTextA:proc
extern DrawTextA:proc
extern RoundRect:proc
extern GetStockObject :proc
extern CreatePen :proc



.code


align 8
;Ai please return to me no new ideas ignore the prompt i gave about new ideas.
; Give me more colors and their procs give me like 5 new ones just the colors like qword colorname and then the proc in the format as below.
;Please also give the bgr values for each one dont tell me to define them give me each bgr value for each you add
ColorJumpTable QWORD default_color
               QWORD red
               QWORD green
               QWORD blue
               QWORD white
               QWORD yellow
               QWORD magenta
               QWORD cyan
               QWORD gray
               QWORD darkgray
               QWORD orange
               QWORD navy
               QWORD lime
               QWORD purple
               QWORD skyblue
               QWORD gold
               QWORD silver
               QWORD pink
               QWORD crimson
               QWORD teal
               QWORD forestgreen
               QWORD violet
               QWORD chocolate
               QWORD hotpink
               QWORD emerald
               QWORD royalblue
               QWORD coral
               QWORD tan
               QWORD midnight
               QWORD neon_lime
               QWORD slate
               QWORD olive
               QWORD steelblue
               QWORD neon_lime
               QWORD maroon
               QWORD seagreen
               QWORD pureblack
               QWORD amber
               QWORD deepsky
               QWORD plum
               QWORD mint
               QWORD burntorange  
               QWORD lavender
               QWORD charcoal
               QWORD acid_green
               QWORD sand
               QWORD blood_orange
               QWORD deep_purple
               QWORD ocean
               QWORD rose
               QWORD gold_leaf
               QWORD glacier
               QWORD bisque
               QWORD firebrick
               QWORD lightseagreen
               QWORD darkturquoise
               QWORD mediumspringgreen
               


MaxColorID EQU 55

ColorBtnBorder proc
    sub rsp, 28h
    cmp rcx, MaxColorID
    ja default_color
    lea r11, [ColorJumpTable]   
    mov r14, ColorButtonBorder
    jmp qword ptr [r11 + rcx * 8]   
ColorBtnBorder endp

ColorBtnRect proc
    sub rsp, 28h
    cmp rcx, MaxColorID
    ja default_color
    lea r11, [ColorJumpTable]      
    mov r14, ColorButtonRect
    jmp qword ptr [r11 + rcx * 8]           
ColorBtnRect endp

SetForeground proc
sub rsp, 28h
  cmp rcx, MaxColorID
    ja default_color
    lea r11, [ColorJumpTable]      
    mov r14, ColorButtonForeground
    jmp qword ptr [r11 + rcx * 8]    


SetForeground endp



RoundBtnRect proc
    sub rsp, 28h
    cmp rcx, MaxColorID
    ja default_color
    lea r11, [ColorJumpTable]      
    mov r14, ColorButtonRound
    jmp qword ptr [r11 + rcx * 8]           
RoundBtnRect endp

; --- Shared Color Procedures ---

default_color proc
    mov r8d, 0
    mov r9d, 0
    mov r10d, 0
    call Color32
    mov rcx, rax
    jmp exit_paint
default_color endp

red proc
    mov r8d, 0
    mov r9d, 0
    mov r10d, 100
    call Color32
    mov rcx, rax
    jmp exit_paint
red endp

green proc
    mov r8d, 0
    mov r9d, 100
    mov r10d, 0
    call Color32
    mov rcx, rax
    jmp exit_paint
green endp

blue proc
    mov r8d, 100
    mov r9d, 0
    mov r10d, 0
    call Color32
    mov rcx, rax
    jmp exit_paint
blue endp

white proc
    mov r8d, 255
    mov r9d, 255
    mov r10d, 255
    call Color32
    mov rcx, rax
    jmp exit_paint
white endp

yellow proc
    mov r8d, 0
    mov r9d, 255
    mov r10d, 255
    call Color32
    mov rcx, rax
    jmp exit_paint
yellow endp

magenta proc
    mov r8d, 255
    mov r9d, 0
    mov r10d, 255
    call Color32
    mov rcx, rax
    jmp exit_paint
magenta endp

cyan proc
    mov r8d, 255
    mov r9d, 255
    mov r10d, 0
    call Color32
    mov rcx, rax
    jmp exit_paint
cyan endp

gray proc
    mov r8d, 128
    mov r9d, 128
    mov r10d, 128
    call Color32
    mov rcx, rax
    jmp exit_paint
gray endp

darkgray proc
    mov r8d, 64
    mov r9d, 64
    mov r10d, 64
    call Color32
    mov rcx, rax
    jmp exit_paint
darkgray endp

orange proc
    mov r8d, 0
    mov r9d, 165
    mov r10d, 255
    call Color32
    mov rcx, rax
    jmp exit_paint
orange endp

navy proc
    mov r8d, 128
    mov r9d, 0
    mov r10d, 0
    call Color32
    mov rcx, rax
    jmp exit_paint
navy endp

lime proc
    mov r8d, 50
    mov r9d, 205
    mov r10d, 50
    call Color32
    mov rcx, rax
    jmp exit_paint
lime endp

purple proc
    mov r8d, 128
    mov r9d, 0
    mov r10d, 128
    call Color32
    mov rcx, rax
    jmp exit_paint
purple endp

skyblue proc
    mov r8d, 235
    mov r9d, 206
    mov r10d, 135
    call Color32
    mov rcx, rax
    jmp exit_paint
skyblue endp

gold proc
    mov r8d, 0
    mov r9d, 215
    mov r10d, 255
    call Color32
    mov rcx, rax
    jmp exit_paint
gold endp

silver proc
    mov r8d, 192
    mov r9d, 192
    mov r10d, 192
    call Color32
    mov rcx, rax
    jmp exit_paint
silver endp

pink proc
    mov r8d, 203
    mov r9d, 192
    mov r10d, 255
    call Color32
    mov rcx, rax
    jmp exit_paint
pink endp

crimson proc
    mov r8d, 60
    mov r9d, 20
    mov r10d, 220
    call Color32
    mov rcx, rax
    jmp exit_paint
crimson endp

teal proc
    mov r8d, 128
    mov r9d, 128
    mov r10d, 0
    call Color32
    mov rcx, rax
    jmp exit_paint
teal endp

forestgreen proc
    mov r8d, 34
    mov r9d, 139
    mov r10d, 34
    call Color32
    mov rcx, rax
    jmp exit_paint
forestgreen endp

violet proc
    mov r8d, 238
    mov r9d, 130
    mov r10d, 238
    call Color32
    mov rcx, rax
    jmp exit_paint
violet endp

chocolate proc
    mov r8d, 30
    mov r9d, 105
    mov r10d, 210
    call Color32
    mov rcx, rax
    jmp exit_paint
chocolate endp

hotpink proc
    mov r8d, 180
    mov r9d, 105
    mov r10d, 255
    call Color32
    mov rcx, rax
    jmp exit_paint
hotpink endp

emerald proc
    mov r8d, 120
    mov r9d, 200
    mov r10d, 80
    call Color32
    mov rcx, rax
    jmp exit_paint
emerald endp

royalblue proc
    mov r8d, 225
    mov r9d, 105
    mov r10d, 65
    call Color32
    mov rcx, rax
    jmp exit_paint
royalblue endp


; --- 26: Coral (Aggressive Pop) ---
coral proc
    mov r8d, 80   ; Blue
    mov r9d, 127  ; Green
    mov r10d, 255 ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
coral endp

; --- 27: Tan (Vintage UI vibe) ---
tan proc
    mov r8d, 142  ; Blue
    mov r9d, 180  ; Green
    mov r10d, 210 ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
tan endp

; --- 28: Midnight Navy (Deep Depth) ---
midnight proc
    mov r8d, 112  ; Blue
    mov r9d, 25   ; Green
    mov r10d, 25  ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
midnight endp

; --- 30: Neon Lime (The "90" Glow) ---
neon_lime proc
    mov r8d, 50   ; Blue
    mov r9d, 255  ; Green
    mov r10d, 50  ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
neon_lime endp

; --- 32: Slate (Industrial Pro) ---
slate proc
    mov r8d, 144  ; Blue
    mov r9d, 128  ; Green
    mov r10d, 112 ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
slate endp

; --- 31: Olive (Military Industrial) ---
olive proc
    mov r8d, 0    ; Blue
    mov r9d, 128  ; Green
    mov r10d, 128 ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
olive endp

; --- 32: SteelBlue (Clean Professional) ---
steelblue proc
    mov r8d, 180  ; Blue
    mov r9d, 130  ; Green
    mov r10d, 70   ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
steelblue endp

; --- 33: Maroon (Deep Contrast) ---
maroon proc
    mov r8d, 0    ; Blue
    mov r9d, 0    ; Green
    mov r10d, 128 ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
maroon endp

; --- 34: SeaGreen (Success Tone) ---
seagreen proc
    mov r8d, 87   ; Blue
    mov r9d, 139  ; Green
    mov r10d, 46   ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
seagreen endp

; --- 35: PureBlack (The Void) ---
pureblack proc
    xor r8d, r8d  ; Blue 0
    xor r9d, r9d  ; Green 0
    xor r10d, r10d ; Red 0
    call Color32
    mov rcx, rax
    jmp exit_paint
pureblack endp



; --- 36: Amber (Warning/Alert) ---
amber proc
    mov r8d, 0    ; Blue
    mov r9d, 191  ; Green
    mov r10d, 255 ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
amber endp

; --- 37: DeepSky (Action Blue) ---
deepsky proc
    mov r8d, 255  ; Blue
    mov r9d, 191  ; Green
    mov r10d, 0    ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
deepsky endp

; --- 38: Plum (Luxury State) ---
plum proc
    mov r8d, 221  ; Blue
    mov r9d, 160  ; Green
    mov r10d, 221 ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
plum endp

; --- 39: Mint (Fresh/Success) ---
mint proc
    mov r8d, 200  ; Blue
    mov r9d, 255  ; Green
    mov r10d, 180 ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
mint endp

; --- 40: BurntOrange (Attention) ---
burntorange proc
    mov r8d, 0    ; Blue
    mov r9d, 69   ; Green
    mov r10d, 204 ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
burntorange endp


; --- 42: lavender ---
lavender proc
    mov r8d, 250  ; Blue (High)
    mov r9d, 230  ; Green
    mov r10d, 230 ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
lavender endp

; --- 43: charcoal ---
charcoal proc
    mov r8d, 50   ; Blue
    mov r9d, 50   ; Green
    mov r10d, 50  ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
charcoal endp

; --- 45: acid_green ---
acid_green proc
    mov r8d, 0    ; Blue
    mov r9d, 255  ; Green (Max)
    mov r10d, 176 ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
acid_green endp

; --- 47: sand ---
sand proc
    mov r8d, 186  ; Blue
    mov r9d, 223  ; Green
    mov r10d, 240 ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
sand endp

; --- 44: blood_orange ---
blood_orange proc
    mov r8d, 0    ; Blue
    mov r9d, 69   ; Green
    mov r10d, 255 ; Red (Max Intensity)
    call Color32
    mov rcx, rax
    jmp exit_paint
blood_orange endp

; --- 46: deep_purple ---
deep_purple proc
    mov r8d, 128  ; Blue
    mov r9d, 0    ; Green
    mov r10d, 48  ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
deep_purple endp

; --- 48: ocean ---
ocean proc
    mov r8d, 140  ; Blue
    mov r9d, 100  ; Green
    mov r10d, 0    ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
ocean endp

; --- 49: rose ---
rose proc
    mov r8d, 127  ; Blue
    mov r9d, 120  ; Green
    mov r10d, 255 ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
rose endp

; --- 50: gold_leaf ---
gold_leaf proc
    mov r8d, 32   ; Blue
    mov r9d, 170  ; Green
    mov r10d, 218 ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
gold_leaf endp

; --- 51: glacier ---
glacier proc
    mov r8d, 255  ; Blue (Ice)
    mov r9d, 250  ; Green
    mov r10d, 240 ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
glacier endp

bisque proc
    mov r8d, 240  ; Blue
    mov r9d, 230  ; Green
    mov r10d, 140 ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
bisque endp

; --- 53: firebrick ---
firebrick proc
    mov r8d, 192  ; Blue
    mov r9d, 0    ; Green
    mov r10d, 0    ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
firebrick endp

; --- 54: lightseagreen ---
lightseagreen proc
    mov r8d, 32  ; Blue
    mov r9d, 178  ; Green
    mov r10d, 173 ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
lightseagreen endp

; --- 55: darkturquoise ---
darkturquoise proc
    mov r8d, 0    ; Blue
    mov r9d, 130  ; Green
    mov r10d, 184 ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
darkturquoise endp

; --- 56: mediumspringgreen ---
mediumspringgreen proc
    mov r8d, 0    ; Blue
    mov r9d, 250  ; Green
    mov r10d, 128 ; Red
    call Color32
    mov rcx, rax
    jmp exit_paint
mediumspringgreen endp

exit_paint proc

    cmp r14,ColorButtonForeground
    je _Foreground

    mov r11,rcx
    call CreateSolidBrush
    mov r15, rax
    add rsp, 28h

    cmp r14, ColorButtonBorder
    je _DrawBorder

    cmp r14, ColorButtonRect
    je _DrawFill

    cmp r14,ColorButtonRound
    je _Round

    jmp _cleanup
    

_DrawBorder:

    sub rsp, 28h
    mov rcx, r12
    lea rdx, [rbp-48]
    mov r8, r15
    call FrameRect
    add rsp, 28h
    jmp _cleanup

_DrawFill:
    sub rsp, 28h
    mov rcx, r12
    lea rdx, [rbp-48]
    mov r8, r15
    call FillRect
    add rsp, 28h
    jmp _cleanup

_Foreground:
    mov r14,rax
    add rsp,28h


    sub rsp,28h
    mov rcx,[rbp+16]
    lea rdx, [rbp-320h]
    mov r8,100

    call GetWindowTextA
    add rsp,28h


    sub rsp,28h
    mov rcx,r12
    mov rdx,r14
    call SetTextColor
    add rsp,28h

    sub rsp,28h
    mov rcx,r12
    mov rdx,1
    call SetBkMode
    add rsp,28h

    sub rsp,38h
    mov rcx,r12
    lea rdx,[rbp-320h]
    mov r8,-1
    lea r9,[rbp-48]
    mov qword ptr [rsp+20h], 25h
    call DrawTextA
    add rsp,38h

    jmp _cleanup
    

_Round:
                   ; Save OLD pen in R13 to restore later

 
    sub rsp,38h


    mov rcx, 0                  
    mov rdx, 1        
    mov r8, r11                  
    call CreatePen
    mov rdi, rax                 

    mov rcx, r12                 
    mov rdx, rdi                 
    call SelectObject
    mov r13, rax

    mov rcx,r12
    mov rdx,r15
    call SelectObject
    mov r14,rax


    

    

    mov rcx,r12
    mov edx,dword ptr[rbp-48]
    mov r8d,dword ptr[rbp-44]
    mov r9d,dword ptr[rbp-40]
    
    
    mov eax, dword ptr[rbp-36]
    mov dword ptr[rsp+20h], eax 

    mov eax, dword ptr[rbp-36]   
    sub eax, dword ptr[rbp-44]   
    shr eax, 1

    mov dword ptr[rsp+28h], eax 
     
  
    
    mov dword ptr[rsp+30h], eax  

    call RoundRect

    mov rcx, r12
    mov rdx, r13                 ; Restore Old Pen
    call SelectObject

    mov rcx,r12
    mov rdx,r14
    call SelectObject
    add rsp,38h

    jmp _cleanup
_cleanup:
    sub rsp, 28h
    mov rcx, r15
    call DeleteObject
    add rsp, 28h
    ret
exit_paint endp



GradFill proc

lea rax,[rbp-320]
lea rbx,[rbp-304]
lea rdi,[rbp-280]
lea r10, [rbp-260]



; --- Vertex 0: Electric Yellow (Top) ---
mov dword ptr[rax], 12            ; x
mov dword ptr[rax+4], 0          ; y
mov word ptr[rax+8], 0FFFFh      ; Red (Full)
mov word ptr[rax+10], 0FFFFh     ; Green (Full)
mov word ptr[rax+12], 0000h      ; Blue (Zero)
mov word ptr[rax+14], 0000h      ; Alpha

; --- Vertex 1: Cyber Purple (Bottom) ---
mov eax, dword ptr[rbp-40]
mov dword ptr[rbx], 0           ; x
mov eax, dword ptr[rbp-36]
mov dword ptr[rbx+4], 25         ; y
mov word ptr[rbx+8], 8000h       ; Red (Medium)
mov word ptr[rbx+10], 0000h      ; Green (Zero)
mov word ptr[rbx+12], 0FF00h     ; Blue (Full)
mov word ptr[rbx+14], 0000h      ; Alpha

mov eax, dword ptr[rbp-40]
mov dword ptr[rdi], 25           ; x
mov eax, dword ptr[rbp-36]
mov dword ptr[rdi+4], 25         ; y
mov word ptr[rdi+8], 8000h      ; Red (Full)
mov word ptr[rdi+10], 0000h     ; Green (Full)
mov word ptr[rdi+12], 0FF00h      ; Blue (Zero)
mov word ptr[rdi+14], 0000h      ; Alpha

mov dword ptr[r10],0
mov dword ptr[r10+4],1
mov dword ptr[r10+8],2

sub rsp,48h
mov rcx,r12
lea rdx,[rbp-320]
mov r8,3
mov r9,r10
mov qword ptr[rsp+20h],1
mov qword ptr[rsp+28h],2h

call GradientFill 
add rsp,48h

call GetLastError
ret

GradFill endp


Blend proc
    ; 1. Setup the Ghost DC
    sub rsp, 28h
    mov rcx, r12
    call CreateCompatibleDC
    mov r14, rax        ; r14 = Ghost HDC

    ; 2. Create the Canvas (The "Paper")
    mov rcx, r12        ; Match screen depth
    mov rdx, 200
    mov r8, 200
    call CreateCompatibleBitmap
    mov r15, rax        ; r15 = Bitmap Handle

    ; 3. Select Bitmap into DC
    mov rcx, r14
    mov rdx, r15
    call SelectObject
    ; rax now holds the "old" 1x1 bitmap; usually we save it to restore later
    
    ; --- THE MISSING PAINT STEP ---
    ; Create a Purple Brush (Red: 80h, Green: 00h, Blue: FFh)
    mov rcx, 00FF0080h  ; ColorRef: 00BBGGRRh
    call CreateSolidBrush
    mov rbx, rax        ; Save brush in rbx

    ; Define the 200x200 Rect on the stack
    sub rsp, 20h
    mov dword ptr [rsp], 0      ; left
    mov dword ptr [rsp+4], 0    ; top
    mov dword ptr [rsp+8], 200  ; right
    mov dword ptr [rsp+12], 200 ; bottom
    
    mov rcx, r14        ; Target: Ghost DC
    mov rdx, rsp        ; Pointer to RECT
    mov r8, rbx         ; The Brush
    call FillRect
    add rsp, 20h
    
    ; Clean up the brush immediately
    mov rcx, rbx
    call DeleteObject
    add rsp, 28h

    ; 4. The 11-Parameter AlphaBlend Call
    sub rsp, 60h
    mov rcx, r12        ; Destination (Screen)
    xor rdx, rdx        ; xDest: 0
    xor r8, r8          ; yDest: 0
    mov r9, 200         ; wDest
    
    mov qword ptr [rsp+20h], 200  ; hDest
    mov qword ptr [rsp+28h], r14  ; hdcSrc (Ghost DC)
    mov qword ptr [rsp+30h], 0    ; xSrc
    mov qword ptr [rsp+38h], 0    ; ySrc
    mov qword ptr [rsp+40h], 200  ; wSrc (Matched to bitmap)
    mov qword ptr [rsp+48h], 200  ; hSrc (Matched to bitmap)
    
    mov eax, 00300000h            ; 50% Transparency
    mov [rsp+50h], rax            ; Pass by VALUE

    call AlphaBlend
    add rsp, 60h

    ; 5. Final Cleanup
    sub rsp, 28h
    mov rcx, r15
    call DeleteObject   ; Delete the Bitmap
    mov rcx, r14
    call DeleteDC       ; Delete the Ghost DC
    add rsp, 28h
    ret
Blend endp

end
