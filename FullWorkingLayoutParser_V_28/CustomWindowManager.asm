...
GlobalArray db 2560 (0)
; Id/Handle qword - 8 bytes
;Rect 4*4 w,h,width,height 16 bytes
;Proc address - 8 bytes
;Parent id - 8 bytes
;State Bitmask 8 bytes
; 84 bytes of padding for now

.code


CreateControl proc
mov [rsp + 8],  rcx  ; id/handle
mov [rsp + 16], rdx  ; w,h
mov [rsp + 24], r8   ; width/height
mov [rsp + 32], r9   ; proc addr
;r15 parent id
;r14 statebitmask
...
_UsedHandle:
    ;rbx holds the invalid handle already defined
    mov rax,1 ; error code already defined handle
