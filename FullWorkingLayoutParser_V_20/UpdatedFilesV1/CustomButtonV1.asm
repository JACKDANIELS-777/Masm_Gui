...
; Versions are relative to where they are added
; =========================================================================================
...
.data
    ...
    point        dq 0,0,0,0,0,0,0 ; X, Y, Direction, W, H, Init, LastX, LastY
...
; -----------------------------------------------------------------------------------------
BtnProc proc
    ; 1. Save parameters into Home Slots
    mov [rsp + 8], rcx           ; hWnd
    mov [rsp + 16], rdx          ; uMsg
...
_OnHover:
    ...
    sub rsp, 30h
    lea rcx, [rbp-64]            ; TME struct
    mov dword ptr [rcx], 24      ; cbSize
    mov dword ptr [rcx+4], 2     ; dwFlags = TME_LEAVE
    mov rax, [rbp+16]            ; hWnd
...
_OnTimer: 
    mov r13, [point+16]          ; Direction state
...
_Done:
    ...
    mov rcx, [rbp+16]          
    xor rdx, rdx
    mov r8, 0                    ; bErase=FALSE (Persistent Fill)
    call InvalidateRect
    add rsp, 28h
    leave
    ret
...
_noskip:
    ...
    lea r9, [rbp-48]
    sub rsp, 30h
    mov dword ptr [rsp+20h], 25h ; DT_CENTER|VCENTER|SINGLELINE
    call DrawTextA
    add rsp, 30h

    sub rsp, 28h
    mov rcx, r15                 ; Last HBRUSH
    call DeleteObject
    add rsp, 28h
